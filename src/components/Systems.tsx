...
const DiagramMultiplayer = () => (
  <svg viewBox="0 0 120 70" fill="none" style={{ width: '100%', height: 70 }}>
    ...
    {([[10, 10], [10, 50], [100, 10], [100, 50]] as [number, number][]).map(([cx, cy], i) => (
      <g key={i}>
...
const DiagramGameplay = () => (
  <svg viewBox="0 0 120 70" fill="none" style={{ width: '100%', height: 70 }}>
    {(['PLAYER', 'PROGRESSION', 'ABILITIES', 'INVENTORY'] as const).map((label, i) => (
      <g key={label}>
...
const DiagramLiveOps = () => (
  <svg viewBox="0 0 120 70" fill="none" style={{ width: '100%', height: 70 }}>
    {(['GAME', 'PlayFab', 'IAP/Ads', 'Analytics'] as const).map((label, i, arr) => (
      <g key={label}>
        <rect x={8 + i * 32} y={20} width={26} height={16} rx="2"
          stroke={i === 1 ? 'var(--color-tick)' : 'var(--color-border)'} strokeWidth="0.9"
          fill={i === 1 ? 'rgba(0,200,220,0.1)' : 'rgba(60,70,90,0.2)'} />
        <text x={8 + i * 32 + 13} y={31} textAnchor="middle" fontFamily="JetBrains Mono" fontSize="5.5"
          fill={i === 1 ? 'var(--color-tick)' : 'var(--color-hud)'}>{label}</text>
        {i < arr.length - 1 && (
          <line x1={8 + i * 32 + 26} y1={28} x2={8 + (i + 1) * 32} y2={28}
...
const SystemTile = ({ system }: { system: System }) => {
